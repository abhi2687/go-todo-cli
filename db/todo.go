...
//
// ANSWER:
// It is a good decision to not expose the fields of a struct because
// it prevents the user from directly modifying the fields of the struct that
// may result in an invalid state of the struct and can result in
// user modfying the struct in a way that is not intended.
type ToDo struct {
	toDoMap    DbMap
	dbFileName string
}
...
func (t *ToDo) RestoreDB() error {
	...
	_, err = io.Copy(newDBFile, backupFile)
	if err != nil {
		return err
	}
...
func (t *ToDo) DeleteItem(id int) error {
	...
	if _, ok := t.toDoMap[id]; !ok {
		return errors.New("todo trying to delete doesnt exists")
	}
...
func (t *ToDo) UpdateItem(item ToDoItem) error {
	...
	if _, ok := t.toDoMap[item.Id]; !ok {
		return errors.New("todo trying to update doesnt exists")
	}
...
func (t *ToDo) GetItem(id int) (ToDoItem, error) {
	...
	loadDBErr := t.loadDB()
	if loadDBErr != nil {
		return ToDoItem{}, loadDBErr
	}

	if _, ok := t.toDoMap[id]; !ok {
		return ToDoItem{}, errors.New("todo trying to fetch doesnt exists")
	}
...
func (t *ToDo) ChangeItemDoneStatus(id int, value bool) error {
	...
	toDoItem, getItemErr := t.GetItem(id)
	if getItemErr != nil {
		return getItemErr
	}
	...
	updateItemErr := t.UpdateItem(toDoItem)
...
func (t *ToDo) loadDB() error {
	...
	// Now empty our map so that we can load the new data
