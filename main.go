...
//						   on how to use it.
//
//	 YOUR ANSWER: processCmdLineFlags define and parses the command line key value
//	 pairs provided as an argument to the program. It uses the flag package to define
//	 allowed flags and their type and then parse, compare and identify flags provided.
//	 If no or invalid flags are provided, it prints the usage and return error message
//	 to main where it prints the error and exists the program.
func processCmdLineFlags() (AppOptType, error) {
	...
	rootCmd.Flags().BoolVar(&restoreDbFlag, "restore", false, "Restore the database from the backup file")
	rootCmd.Flags().BoolVarP(&listFlag, "l", "l", false, "List all the items in the database")
	rootCmd.PersistentFlags().IntVarP(&queryFlag, "q", "q", 0, "Query an item in the database")
	rootCmd.Flags().StringVarP(&addFlag, "a", "a", "", "Add an item to the database")
	rootCmd.PersistentFlags().StringVarP(&updateFlag, "u", "u", "", "Update an item in the database")
	rootCmd.Flags().IntVarP(&deleteFlag, "d", "d", 0, "Delete an item from the database")
	rootCmd.Flags().BoolVarP(&itemStatusFlag, "s", "s", false, "Change item 'done' status to true or false, query flag required")
	...
	rootCmd.Run = func(cmd *cobra.Command, args []string) {
		cmd.Flags().Visit(func(f *pflag.Flag) {
			...
			case "s":
				...
				// check q flag exists
				qFlag, err := cmd.Flags().GetInt("q")
				if qFlag == 0 || err != nil {
					fmt.Println("The -s option requires the -q option to also be set")
	...
	err := rootCmd.Execute()
	if err != nil {
		fmt.Println("Error while executing rootCMD", err)
		os.Exit(1)
	}
